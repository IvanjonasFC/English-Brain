...
/// or unreachable.
final interviewPacksProvider = FutureProvider<List<InterviewPack>>((ref) async {
  final api = ref.watch(apiClientProvider);
  final cache = ref.watch(localCacheProvider);
  const cacheKey = AppConstants.keyCacheInterview;

  List<InterviewPack> parse(List<dynamic> raw) =>
      raw.map((e) => InterviewPack.fromJson(e as Map<String, dynamic>)).toList();
  ...
  Future<void> revalidate() async {
    try {
      final res = await api.dio.get('/api/packs/interview');
      final data = res.data;
  ...
  try {
    final decoded = jsonDecode(await rootBundle.loadString('assets/seed/interview.json'));
  ...
  unawaited(revalidate());
  return ContentSeeds.interviewPacks;
});
...
Future<void> refreshInterview(WidgetRef ref) async {
  ...
  try {
    ...
    final data = res.data;
    if (data is List && data.isNotEmpty) {
      await cache.saveContentList(AppConstants.keyCacheInterview, data);
    }
  } catch (_) {}
  ref.invalidate(interviewPacksProvider);
  await ref.read(interviewPacksProvider.future);
}
