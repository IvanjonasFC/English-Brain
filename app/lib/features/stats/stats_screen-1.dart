...
class _StatsScreenState extends ConsumerState<StatsScreen> {
  ...
  Widget _buildPracticeSummaryCard() {
    ...
    for (final e in _journalEntries) {
      // JournalEntry no tiene 'category'; el área se infiere de title + sessionId.
      byArea.update(_areaOf('${e.title} ${e.sessionId}'), (v) => v + 1, ifAbsent: () => 1);
    }
  ...
  String _areaOf(String category) {
    final c = category.toLowerCase();
    if (c.contains('gram')) return 'Gramática';
    if (c.contains('lectura') || c.contains('comprens') || c.contains('inmersi')) return 'Comprensión';
    if (c.contains('vocab')) return 'Vocabulario';
    if (c.contains('fsrs') || c.contains('repaso')) return 'Repaso';
    if (c.contains('entrevista') || c.contains('simulaci') || c.contains('star')) return 'Speaking';
    return 'Otros';
  }
